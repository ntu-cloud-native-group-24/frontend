...
const OrderFoodDisplay = ({ food }: FoodDisplayProps) => {
    const InnerDisplay = () => {
        if (
            food.singleSelections.length === 0 &&
            food.multipleSelections.length === 0
        ) {
...
const OrderMobileDisplay = ({ order, orderState }: OrderProps) => {
    ...
    return order.order_state === orderState ? (
        <Card
            ...
            style={{ width: "full" }}
            className={
                order.order_state === OrderState.CANCELED ? "bg-red-400" : ""
            }
        >
