...
const ContentLayout = (items: ContentProps) => {
    ...
    const BreadCrumbItems = useMemo(() => {
        return items.subMenuName === '' ? [
            {title: (<><FolderOutlined /><span>目錄</span></>)}, 
            {title: (<>{items.currentIcon}<span>{items.currentName}</span></>)}
        ] : [
            {title: (<><FolderOutlined /><span>目錄</span></>)}, 
            {title: (<>{items.subMenuIcon}<span>{items.subMenuName}</span></>)},
    ...
    return (
        <div className="m-3 h-5/6">
            ...
            <ContentHeader 
                icon={items.subMenuName === '' ? items.currentIcon : items.subMenuIcon} 
