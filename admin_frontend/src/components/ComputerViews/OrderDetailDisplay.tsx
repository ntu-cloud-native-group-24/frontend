import { Button, Card, Flex, Typography, Statistic, Divider, Image, } from "antd"
import { DeliveryMethod, OrderDetailProps, OrderState, PaymentType } from "../../interfaces/OrderInterface"
import { useState, useMemo } from "react"
import { FoodDisplayProps, fallbackSRC } from "../../interfaces/FoodInterface"

const OrderFoodDisplay = ({ food } : FoodDisplayProps ) => {
    
    const totalMoney = food.price + food.singleSelections.reduce((acc, v) => acc + v.selections.reduce((acc, v) => acc + v.price, 0), 0) + food.multipleSelections.reduce((acc, v) => acc + v.selections.reduce((acc, v) => acc + v.price, 0), 0)

    return (
        <Flex justify="space-between" align="center">
            <Flex gap='large' align="center">
                <Image src={food.picture} fallback={fallbackSRC}
                       width={96} height={96}        
                />
                <Flex vertical gap='small'>
                    <Typography.Text>{food.name}</Typography.Text>
                    <Typography.Text className="opacity-60 text-[11px]">x1</Typography.Text>
                </Flex>
            </Flex>
                ...
                {food.singleSelections.map((selection) => (
                    <Flex vertical>
                        ...
                        {selection.selections.map((childSelection) => (
                            <p className="opacity-70 text-[12px]">{childSelection.name} (${childSelection.price})</p>
                        ))}
                    </Flex>
                ))}
            ...
            <Flex justify="center" gap={4}>
                ...
                <Typography.Text className="text-md">{totalMoney}</Typography.Text>
            </Flex>
        </Flex>
    )
}
...
const OrderDetailDisplay = ({ order } : OrderDetailProps) => {
    ...
    const [userPicture, setUserPicture] = useState(order ? order.user.picture_url : fallbackSRC)

    //TODO: check if order.price is already totalAmount or what
    const totalMoney = order ? order.foods.reduce((acc, v) => acc = acc + v.price + v.singleSelections.reduce((acc, v) => acc + v.selections.reduce((acc, v) => acc + v.price, 0), 0) + v.multipleSelections.reduce((acc, v) => acc + v.selections.reduce((acc, v) => acc + v.price, 0), 0), 0) : 0
        ...
        <Flex vertical gap='middle' className="pl-8 pr-8 w-full">
            ...
            <Card 
                ...
                title={(<Flex vertical className="w-3/4 pt-8 pb-8">
                    ...
                    <p className="text-[10px] opacity-50">{order.timestamp.toDateString()}</p>
                    </Flex>)}
                ...
                <Flex vertical>
                        ...
                        <Flex vertical gap='large'>
                            {
                                order.foods.map((food) => (
                                    <>
                                        ...
                                        {food === order.foods[order.foods.length - 1] ? <></> : <Divider dashed />}
                                    </>
                                ))
                            }
                        </Flex>
                    ...
                    <Flex justify="space-between" align="center">
                        ...
                        <Flex justify="center" gap={4}>
                            ...
                            <Typography.Text className="text-xl">{totalMoney}</Typography.Text>
                        </Flex>
                    </Flex>
                </Flex>
            </Card>
            <Flex gap='large' justify="flex-end" align="center">
                <Button type='primary' danger>Cancel Order</Button>
